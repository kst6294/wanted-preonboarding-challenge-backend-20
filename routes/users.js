...
// 예약중인 용품
// 구매자/판매자로서
router.get("/reserved-list", verifyToken, transactionHandler, async (req, res, next) => {
    const buyer_id = req.decoded.id;
    const seller_id = req.decoded.id;

    try {
        const orders = await show_reserved_orders({ buyer_id, seller_id }, req.transactionProvider);
