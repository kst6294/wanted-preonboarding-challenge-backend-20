...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemPurchaseResponseDTO purchaseItem(ItemPurchaseRequestDTO itemPurchaseRequestDTO, Long id) {
        ...
        Item item = itemRepository.findByIdFetchJoinMember(itemPurchaseRequestDTO.getId())
                .orElseThrow(() -> new ItemException(ItemExceptionInfo.NOT_FOUND_ITEM, currentMember.getId() + "번 유저가 "
                        + itemPurchaseRequestDTO.getId() + "번 상품 구매 시도.(상품 존재하지 않음)"));

        // 본인의 상품은 구매 불가
        if (item.getMember() == currentMember) {
            throw new ItemException(ItemExceptionInfo.DONT_PURCHASE_SELF_ITEM, currentMember.getId() + "번 유저가 "
                    + item.getId() + "번 상품 구매 신청을 실패했습니다.(본인 물건 구매)");
        }
