...
public interface OrderRepository extends JpaRepository<Orders, Long> {
    ...
    @Query("select o, i from Orders o join Item i on o.item.id = i.id where o.member.id = :memberId and o.item.id = :itemId")
    Optional<Orders> findOrderHistory(Long memberId, Long itemId);

    @Query("select o, i from Orders o join Item i on o.item.id = i.id where o.item.id = :itemId")
    List<Orders> findOrdersByItemId(Long itemId);
